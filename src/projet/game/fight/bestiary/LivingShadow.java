...
public class LivingShadow implements Bestiary,Effects{
	...
	public LivingShadow(int health) {
		this.imgPath = "Living_Shadow.png";
		this.health = health;
		this.protection = 0;
		this.maxHealth = 50;
		this.isAlive = true;
		this.xp = 20;
		this.dmg = new int[]{15, 19};
		this.nextAction = prediction();
		this.description = "A dark shadow of yourself. Doppel-get-over-it!";
	}
	...
	public void sleep(Hero hero, int sleep) {
		hero.setSleep(sleep);
	}
	...
	public void turn(GameModel gameData, Hero hero){
		...
		if (sleep == 0) {
			...
			if (action.equals("Attack")) {
				...
				sleep(hero,nextAction.get(action));
			}
		}
		// À la fin du tour
	...
	public Bestiary copie() {
		return new LivingShadow(health);
	}
