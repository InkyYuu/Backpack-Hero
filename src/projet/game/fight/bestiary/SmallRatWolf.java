...
public class SmallRatWolf implements Bestiary,Effects{
	...
	public SmallRatWolf(int health) {
		this.imgPath = "Small_Ratword_Animated.gif";
		this.health = health;
		this.protection = 0;
		this.maxHealth = 32;
		this.isAlive = true;
		this.xp = 6;
		this.dmg = new int[]{7, 9};
		this.shield = new int[]{14};
		this.nextAction = prediction();
		this.description = "The smaller version of the ratwolves that patrol the upper layers of the dungeon";
	}
	...
	public void protect() {
		if (this.shield.length == 1) { 
			int protection = this.shield[0];
			this.shield = new int[]{protection + haste - slow};
		} else if (this.shield.length == 2) { 
			int minShield = this.shield[0];
			int maxShield = this.shield[1];
			int protection = (int) (Math.random() * (maxShield - minShield + 1)) + minShield; 
	...
	public Bestiary copie() {
		return new SmallRatWolf(health);
	}
